...
export default function App(props) {
    return (
        <Canvas shadows dpr={[1, 2]} camera={{ position: [-1, 1.5, 2], fov: 25 }} style={{ 'height': '100vh', }}>
            <spotLight position={[-4, 4, -4]} angle={0.06} penumbra={1} castShadow shadow-mapSize={[2048, 2048]} />
            <Suspense fallback={null}>
                <Track position-z={-0.25} url="/sound/synth.mp3" />
                <Track position-z={0} url="/sound/snare.mp3" />
                <Track position-z={0.25} url="/sound/drums.mp3" />
                {/* <Zoom url="/sound/drums.mp3" /> */}
            </Suspense>
...
function Track({ url, y = 2500, space = 1.8, width = 0.01, height = 0.05, obj = new THREE.Object3D(), ...props }) {
    ...
    console.log(gain, context, update, data);
    useEffect(() => {
